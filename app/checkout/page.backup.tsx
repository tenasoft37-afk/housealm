...
export default function CheckoutPage() {
  ...
  const validatePhoneNumber = (phone: string): boolean => {
    // Simple regex validation: +971 followed by 1-2 digits, then 3 digits, then 3 digits
    // Allows optional spaces
    const phoneRegex = /^\+971\s?\d{1,2}\s?\d{3}\s?\d{3}$/;
  ...
  const handlePhoneInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Auto-prepend +971 if missing
    if (!value.startsWith("+971") && value.length > 0) {
      // Remove any existing +971 or partial +971
      value = value.replace(/^\+?9?7?1?/, "");
      value = "+971" + value;
    }
  ...
  const handlePhoneBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (value.trim()) {
      if (!validatePhoneNumber(value)) {
        setErrors((prev) => ({
          ...prev,
          phoneNumber: "Please enter a valid phone number (+971 …)",
        }));
      } else {
        setErrors((prev) => {
          const newErrors = { ...prev };
  ...
  const validateForm = () => {
    ...
    if (!formData.phoneNumber.trim()) {
      ...
      newErrors.phoneNumber = "Please enter a valid Lebanese phone number (+971 …)";
    }
    if (!formData.governorate) {
...
Payment Method: Cash on Delivery`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const whatsappUrl = `https://wa.me/971501916610?text=${encodedMessage}`;
